...
function ConnectionHandler($io, $socket, $serverState) {
  ...
  function jsonify(player, redactDetails) {
    ...
    // console.log(player);
    result.cars = [...result.cars.values()];
    result.dcCards = [...result.dcCards.values()];
    result.insurances = [...result.insurances.values()];
  ...
  function getGameState() {
    // console.log("mu? ", manager.users);
    let users = _.reject(manager.users, { id: userId });
    users.forEach(function (user) {
      console.log("1: ", user.player);
