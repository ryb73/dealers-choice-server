...
function RockPaperScissors($game, $players, $winCounter) {
  ...
  function giveAnswer(player, answer) {
    // If we've created a new instance to handle the
    // conclusion (i.e. do-overs or showdowns), then
    // forward the answer to the new instance
    if(rpsDelegate) {
      rpsDelegate.giveAnswer(player, answer);
      return;
    }
  ...
  function restartGame(players) {
    rpsDelegate = new RockPaperScissors(game, players, winCounter);

    q.delay(RESULTS_DELAY)
      .then(function() {
        deferred.resolve(rpsDelegate.handleIt(broadcast));
      });
  }
